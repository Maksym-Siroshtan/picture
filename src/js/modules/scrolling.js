const scrolling = (upSelector) => {
  ...
  //Реализация плавного скролла RequestAnimationFrame. Модуль универсальный реализован для всех локальных ссылок(#). При желании можно реализовать на какую-то определённую ссылку с определённым направлением при помощи слушателя событий addEventListener на нужный нам элемент.
  ...
  links.forEach((link) => {
    link.addEventListener("click", function (event) {
      try {
        ...
        //let scrollTop = Math.round(element.documentElement.scrollTop || document.body.scrollTop); Таким образом нужно получать scrollTop для поддержки старых браузеров.


        let widthTop = document.documentElement.scrollTop, //Определим какое расстояние было пролистано сверху.
        ...
        const step = (time) => {
          ...
          //Тут (widthTop - progress / speed) мы используем значение того сколько пользователь уже пролистал(progress), делим на скорость(speed) с которой у нас происходит анимация и всё это отнимаем от значения widthTop(scrollTop) нашей страницы.
  ...
  /* //Реализайия плавного скролла Pure js. Модуль универсальный реализован для всех локальных ссылок(#). При желании можно реализовать на какую-то определённую ссылку с определённым направлением при помощи слушателя событий addEventListener на нужный нам элемент.
  
  const links = document.querySelectorAll('[href^="#"]'), //Все локальные ссылки.
    element = document.documentElement, //html.
